...
/* Adaptors for the libsodium code. */
std::uint32_t LOAD32_LE(std::uint8_t const* src) {
	return (std::uint32_t(src[0]) << 0)
	     | (std::uint32_t(src[1]) << 8)
	     | (std::uint32_t(src[2]) << 16)
	     | (std::uint32_t(src[3]) << 24)
	     ;
}
void STORE32_LE(std::uint8_t* dst, std::uint32_t src) {
	dst[0] = std::uint8_t(src >> 0);
	dst[1] = std::uint8_t(src >> 8);
	dst[2] = std::uint8_t(src >> 16);
	dst[3] = std::uint8_t(src >> 24);
}
std::uint32_t ROTL32(std::uint32_t x, int b) {
	return (x << b) | (x >> (32 - b));
}
...
chacha20_ietf( std::uint8_t* out
	...
	(void) stream_ietf_ext_ref(out, out_len, key, nonce);
}
