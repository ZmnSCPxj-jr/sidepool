...
* TLV fields `swap_party_expand_state_nonces` and
  `swap_party_expand_state_next` exist and have valid
  lengths.
...
The pool follower now determine how the recreated update
transactions look like, based on the next output state.
...
* TLV fields `swap_party_contract_state_nonces` and
  `swap_party_contract_state_next` exist and have valid
  lengths.
...
If it already has generated a partial signature, the pool
follower:
